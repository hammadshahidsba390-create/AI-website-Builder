    ...
    <>
      <nav className="z-50 flex items-center justify-between w-full py-4 px-4 md:px-16 lg:px-24 xl:px-32 backdrop-blur border-b text-white border-slate-800">
        ...
        <div className="flex items-center gap-3">
          ...
          <button id="open-menu" type="button" title="Open menu" className="md:hidden active:scale-90 transition" onClick={() => setMenuOpen(true)} >
      ...
      {menuOpen && (
        <div className="fixed inset-0 z-[100] bg-black/60 text-white backdrop-blur flex flex-col items-center justify-center text-lg gap-8 md:hidden transition-transform duration-300">
          ...
          <Link to="/Projects" onClick={() => setMenuOpen(false)}>Myprojects</Link>
          <Link to="/Community" onClick={() => setMenuOpen(false)}>Community</Link>
          <Link to="/Pricing" onClick={() => setMenuOpen(false)}>Pricing</Link>

          <button type="button" title="Close menu" className="active:ring-2 active:ring-white aspect-square size-10 p-1 items-center justify-center bg-slate-100 hover:bg-slate-200 transition text-black rounded-md flex" onClick={() => setMenuOpen(false)} >
